...
;(princ (get-connected 1 (make-edge-list)))
...
(defun find-islands (nodes edge-list)
  (let ((islands nil))
    (labels ((find-island (nodes)
             ...
             (find-island nodes))
            islands))
;(princ (find-islands  (loop for i from 1 to *node-num* collect i) (make-edge-list)))
...
(defun make-city-edges ()
  (let* ((nodes (loop for i from 1 to *node-num* collect i))
         ...
         (cops (remove-if-not (lambda (x)
                                (zerop (random *cop-odds*))) edge-list)))
...
(defun neighbors (node edge-alist)
 ...
 (defun make-city-nodes (edge-alist)
   (let ((wumpus (random-node))
     ...
     (loop for n from 1 to *node-num* collect (append (list n) 
